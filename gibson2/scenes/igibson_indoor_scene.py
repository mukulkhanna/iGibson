...
import random
import json
from gibson2.utils.assets_utils import (
    get_ig_scene_path,
    get_ig_model_path,
...
class InteractiveIndoorScene(StaticIndoorScene):
    ...
    def __init__(
        ...
        self.custom = custom
        self.split = split
        self.object_splits = object_splits
        ...
        for link in self.scene_tree.findall("link"):
            if "category" in link.attrib:
                    ...
                    if model == "random":
                        ...
                        if "random_group" in link.attrib:
                            ...
                            # select a random model and cache it
                                ...
                                if self.object_splits is not None:
                                    model = random.choice(
                                        [
                                            x
                                            for x in os.listdir(category_path)
                                            if x
                                            in self.object_splits[self.split][category]
                                        ]
                                    )
                                else:
                            ...
                            if self.object_splits is not None:
                                model = random.choice(
                                    [
                                        x
                                        for x in os.listdir(category_path)
                                        if x in self.object_splits[self.split][category]
                                    ]
                                )
                            else:
                                model = random.choice(os.listdir(category_path))
                ...
                joint_xyz = np.array(
                    [
                        float(val)
                        for val in joint_connecting_embedded_link.find("origin")
                        .attrib["xyz"]
                        .split(" ")
                    ]
                )
                ...
                joint_parent = joint_connecting_embedded_link.find("parent").attrib[
                    "link"
                ]
        ...
        if self.object_splits is not None:
            self.split_scenes_path = os.path.join(
                scene_dir, "urdf", f"{scene_id}_split_{self.inst_id}.urdf"
            )
            self.scene_tree.write(self.split_scenes_path)
            self.scene_file = self.split_scenes_path

    def filter_rooms_and_object_categories(
        self, load_object_categories, load_room_types, load_room_instances
    ):
        """
        ...
        if not self.custom:
            ...
            if joint_type != "floating":
                new_joint = ET.SubElement(
                    added_object.object_tree.getroot(),
                    "joint",
                ...
                ET.SubElement(
                    ...
                    dict(
                        [
                            ("rpy", "{0:f} {1:f} {2:f}".format(*orientation_rpy)),
                            ("xyz", "{0:f} {1:f} {2:f}".format(*position)),
                        ]
                    ),
                )
                ET.SubElement(new_joint, "parent", dict([("link", joint_parent)]))
                ET.SubElement(new_joint, "child", dict([("link", object_name)]))
                ET.SubElement(
                    added_object.object_tree.getroot(),
                    "link",
    ...
    def check_scene_quality(self, body_ids, fixed_body_ids, generous=False):
        """
        ...
        :param generous: whether to allow minimal no. of collisions in scene quality check
        ...
        for body_a, body_b in overlapped_body_ids:
            ...
            if generous:
                # allowing if n_collision_points < 2
                if n_collisions < 2:
                    has_collision = False
                else:
                    has_collision = True
            else:
                has_collision = n_collisions > 0

            quality_check = quality_check and (not has_collision)
            if has_collision:
                n_collision_points.append(n_collisions)
                body_body_collision.append((body_a, body_b))

        if generous:
            # allowing if only one obj-obj collision in whole scene (w < 50 collision pts)
            if len(body_body_collision) == 1:
                if n_collision_points[0] < 50:
                    body_body_collision.pop(0)
                    n_collision_points.pop(0)
                    has_collision = False
                    quality_check = True
        ...
        for body_a, body_b in body_body_collision:
            logging.warning(
                "scene quality check: {} and {} has collision.".format(
                    body_id_to_name[body_a], body_id_to_name[body_b],
                )
            )
